...
!define UNINSTALLER_NAME "kq3_heb_uninstaller.exe"
...
Name "התרגום העברי של KQ3"

OutFile "kq3-hebrew-installer.exe"
...
• יש ברשותך עותק תקין של KQ3 בגרסה המקורית עם ממשק הקלדת פקודות (ניתן לקנות ב GoG)$\r$\n \
• מותקנת גרסת Daily של ScummVM" ;" ; gvim get's confused without that extra "

!define MUI_FINISHPAGE_TEXT  "ההתקנה הושלמה בהצלחה.$\r$\n$\r$\n \
• כעת ניתן להוסיף את המשחק ל ScummVM, והוא יזוהה כ KQ3 בעברית.$\r$\n \
...
DirText "בחר את התיקייה שקבצי KQ3 נמצאים בה"
...
Section "Update file"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KQ3_Hebrew" \
...
Section "Uninstall"
    ...
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KQ3_Hebrew"
SectionEnd
